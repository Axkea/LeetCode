package binarySearch;

import dp.L1187Solution;

import java.util.Arrays;
import java.util.concurrent.ThreadPoolExecutor;

public class Test {
    public static void main(String[] args) {
//        L153Solution solution=new L153Solution();
//        int[] data={3,1,2};
//        System.out.println(solution.findMin(data));
//        L154Solution solution=new L154Solution();
//        int[] data={2,2,2,0,0,1};
//        System.out.println(solution.findMin(data));
//        BinarySearch binarySearch = new BinarySearch();
//        int[] data={1,2,2,2,3};
//        System.out.println(binarySearch.search2(data, 2));
//        System.out.println(binarySearch.search3(data, 2));
//        L69Solution solution=new L69Solution();
//        System.out.println(solution.mySqrt(2147483647));
//        S11Solution solution=new S11Solution();
//        int[] data={3,3,1,3};
//        System.out.println(solution.minArray(data));
        M1003Solution solution=new M1003Solution();
        int[] data={15,16,19,20,25,1,3,4,5,7,10,14};
        System.out.println(solution.search(data, 5));
    }
}
